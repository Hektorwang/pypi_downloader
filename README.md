...
- ✅ **Fast**: Async concurrent downloads (16 streams by default) + thread pool for I/O (never blocks event loop)
...
- **Async & concurrent** – hundreds of files in parallel without blocking (default: 16 streams, configurable)
...
# After (--latest-patch):  numpy 1.19.5, 1.20.3, 1.21.6, 1.22.4, ..., 1.26.4 (~20 versions)
```
...
**When to use:**
- Building internal mirrors with limited storage/bandwidth
- When you trust semantic versioning (patch = bug fixes only)
- For most production use cases

**When NOT to use:**
- When you need specific patch versions for bug workarounds
- When packages don't follow semantic versioning strictly
